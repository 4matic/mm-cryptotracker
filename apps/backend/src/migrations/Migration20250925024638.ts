...
export class Migration20250925024638 extends Migration {
  ...
  override async up(): Promise<void> {
    ...
    this.addSql(`create table "trading_pair" ("id" serial primary key, "base_asset_id" int not null, "quote_asset_id" int not null, "symbol" varchar(255) not null, "slug" varchar(255) not null, "is_active" boolean not null default true, "is_visible" boolean not null default false, "created_at" timestamptz not null, "updated_at" timestamptz not null);`);
    this.addSql(`alter table "trading_pair" add constraint "trading_pair_symbol_unique" unique ("symbol");`);
    this.addSql(`alter table "trading_pair" add constraint "trading_pair_slug_unique" unique ("slug");`);
